...
module.exports = {
  async handler(chatUpdate) {
    ...
    try {
      ...
      let RName = this.getName(m.sender)
      let usedPrefix
...
global.dfail = (type, m, conn) => {
  let msg = {
    ...
    mods: '*😼 This command can only be used by the Cat Moderators! 😼*',
    premium: '*😼 This command can only be used by the Cat Premium members! 😼*',
    group: '*😿 This command can only be used in Groups! 😿*',
    private: '*😿 This command can only be used in Private Chats! 😿*',
    admin: '*😿 This command can only be used by the Group Admins! 😿*',
    botAdmin: '*😼 Make sure that the, Cat is an Admin in this Group! 😿*',
    unreg: '😺 Hi {name},\nYou are not registered in my friend list 😿\nPlease register to use me 😺\n😸 To register type:\n*{p}reg Name/Age*\nExample:\n{p}reg ToxicDevil/22'.replace('{name}', RName).replace('{p}', usedPrefix),
    nsfw: '😸 NSFW is not activated here'
  }[type]
