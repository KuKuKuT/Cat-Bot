...
let authFile = `${opts._[0] || 'cat'}.session.json`
...
global.reloadHandler = function () {
  ...
  conn.bye = '😼 RIP @user!'
  conn.spromote = '*Congratulations 😽 @user is now an Admin!*'
  conn.sdemote = '*Congratulations 😽 @user is no longer an Admin!*'
...
async function _quickTest() {
  ...
  if (!s.ffmpeg) conn.logger.warn('please install FFMPEG')
  if (s.ffmpeg && !s.ffmpegWebp) conn.logger.warn('Some Commands Will Not Work With Out Necessary Packages')
  if (!s.convert && !s.magick && !s.gm) conn.logger.warn('imagemagic, ffmpeg are most necessary packages install it!')
}
